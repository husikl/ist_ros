...
def mask2points(mask):
    ret, labels, stats, centroids = cv2.connectedComponentsWithStats(mask, connectivity=8)
...
class InferenceCore:
    ...
    def step(self, image, mask=None, valid_labels=None, end=False):
        ...
        need_segment = (self.curr_ti > 0) and ((valid_labels is None) or (len(self.all_labels) != len(valid_labels))) # ! when comes new labels or no labels given 
        is_deep_update = (
        ...
        if mask is not None:
            ...
            pred_prob_with_bg = aggregate(mask, dim=0) # engage previded mask with prediction for previous objects
