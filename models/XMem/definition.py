"""
The following is the original network definition for XMem maintained by hkchengrex
...
The file structure is reorganized to fit the CREST project. Some functions are removed.
By Jetyau, contact for more information.
...
class XMem(nn.Module):
    def __init__(self, config, model_path=None, map_location=None):
        ...
        """
        print('XMem is selected to tracking masks')
        super().__init__()
    ...
    def encode_value(self, frame, image_feat_f16, h16, masks, is_deep_update=True): 
        ...
        if num_objects != 1:
            others = torch.cat([
                ...
                , dim=1, keepdim=True)
            for i in range(num_objects)], 1) # ! 1 vs others mask
        else:
